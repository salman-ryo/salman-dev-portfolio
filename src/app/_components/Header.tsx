        ...
        <ul className="flex flex-wrap sm:flex-nowrap justify-center items-center gap-y-1 sm:gap-5 w-[22rem] sm:w-[initial] text-[0.9rem] font-medium text-gray-600">
          {links.map((link) => (
            ...
            >
                ...
                >
              <Link href={link.hash} className={`${activeTab === link.name? "text-gray-950 font-semibold transition" : null} w-full flex justify-center items-center p-3 hover:text-gray-950 transition cursor-pointer`}>
                  {link.name}
                  {link.name === activeTab &&(
                    <motion.span className="-z-10 bg-gray-300 rounded-full absolute inset-0"
                    layoutId="activeTab"
                    transition={{
                      type:'sping',
                      stiffness:400,
